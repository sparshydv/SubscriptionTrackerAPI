...
import { JWT_SECRET, JWT_EXPIRES_IN, NODE_ENV, GOOGLE_CLIENT_ID } from "../config/env.js";
...
export const googleSignIn = async (req, res, next) => {
    try {
        ...
        if (!user) {
            const salt = await bcrypt.genSalt(10);
            const hashedPassword = await bcrypt.hash(Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8), salt);
